...
export default function App() {
  ...
  // Shared logic to build an Expense object from current form state
  const buildExpense = async (existingId?: string, existingDate?: string): Promise<Expense> => {
  ...
  const handleSaveEdit = async () => {
    ...
    if (isNaN(amountNum) || amountNum <= 0) return
    setSaving(true)
    const updated = await buildExpense(editingExpense.id, editingExpense.date)
  ...
  const handleDeleteExpense = (expenseId: string) => {
    ...
    // If we were in edit view, go back to detail
    if (view === 'edit-expense') {
      setEditingExpense(null)
      setView('detail')
    }
  }
  ...
  // ── Shared expense form UI (used by both Add and Edit views) ───────────────
        ...
        <div className="topbar">
          ...
          {isEdit ? (
            <button
              ...
              onClick={() => setDeleteExpenseConfirm(editingExpense!.id)}>
              🗑 Delete
            </button>
          ) : (
            <div style={{ width: 64 }} />
          ...
          <div className="card animate-in" style={{ textAlign: 'center', padding: '20px 20px 16px', marginBottom: 16 }}>
            ...
            {isEdit && editingExpense && (
              <div style={{ fontSize: 11, color: '#b0a898', marginTop: 6 }}>
                Originally logged {formatDate(editingExpense.date)} · {fmt(editingExpense.amountHome, activeTrip.homeCurrency)}
              </div>
            )}
          </div>
                      ...
                      {exps.map(e => (
                        <div key={e.id} className="expense-row"
                          ...
                          <div style={{ textAlign: 'right', flexShrink: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
                            ...
                            <span style={{ color: '#c4bdb4', fontSize: 14, flexShrink: 0 }}>›</span>
                          </div>
                        </div>
                      ))}
                    </div>
  ...
  if (view === 'add' && activeTrip) {
    return renderExpenseForm(false)
  }

  // ── EDIT EXPENSE ───────────────────────────────────────────────────────────
  if (view === 'edit-expense' && activeTrip && editingExpense) {
    return renderExpenseForm(true)
  }
